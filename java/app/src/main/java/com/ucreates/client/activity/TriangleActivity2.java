...
import com.ucreates.client.behaviour.TriangleBehaviour2;
...
public class TriangleActivity2 extends AppCompatActivity implements GLSurfaceView.Renderer {
    private GLES1Renderer renderer;
    private ArrayList<TriangleBehaviour2> behaviours;
    @Override
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        this.behaviours = new ArrayList<TriangleBehaviour2>();
        for (int i = 0; i < 1; i++) {
            this.behaviours.add(new TriangleBehaviour2(context));
        }
        this.renderer = new GLES1Renderer();
        this.renderer.create();
        this.renderer.camera.setClear(GLESColor.black);
        this.renderer.camera.setClippingPlane(-100.0f, 100.0f, GLES1Renderer.DIMENSION2D);
        return;
    }
    @Override
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        for (int i = 0; i < this.behaviours.size(); i++) {
            TriangleBehaviour2 behaviour = this.behaviours.get(i);
